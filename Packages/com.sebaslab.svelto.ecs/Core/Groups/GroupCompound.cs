...
//                };
                ...
                if (range == 0)
                {
                    group = new ExclusiveGroup(bitmask);
                    _Groups.Add(group);
                    range = 1;
                }
                else
                {
                    group = new ExclusiveGroup(range, bitmask);
                    for (int i = 0; i < range; i++)
                    {
                        _Groups.Add(group + (uint)i);
                    }
                }
#if DEBUG && !PROFILE_SVELTO
                ...
                {
                    var name = $"Compound: {typeInfo.Name} ID {(uint)group.id + i}";
        ...
        public static ExclusiveBuildGroup BuildGroup => new ExclusiveBuildGroup(_Groups[0], range);
